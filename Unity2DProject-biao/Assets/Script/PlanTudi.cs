...
public class PlanTudi : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dt3 = LoadDateTime("tu1");
        dtpotu = LoadDateTime("dtpotu");
        dtfaya = LoadDateTime("dtfaya");
        dtzhangye = LoadDateTime("dtzhangye");
        dtkaihua = LoadDateTime("dtkaihua");
        dtjieguo = LoadDateTime("dtjieguo");

        type = PlayerPrefs.GetInt("type");
        dt1 = DateTime.Now;


        Pet1 = LoadGameObject("Pet1");
        if(Pet1 != null)
        {
            Pet1mount = false;
    ...
    void Update()
    {
        ...
        if ( type==0)
        {
            ...
            if (Input.GetKeyDown(KeyCode.F) && inTudi)
            {
                ...
                SaveDateTime("tu1", dt3);
              
                dtpotu = dt3.AddSeconds(-12f);

                dtfaya = dt3.AddSeconds(-9f);
                dtzhangye = dt3.AddSeconds(-6f);
                dtkaihua = dt3.AddSeconds(-3f);
                dtjieguo = dt3;
                SaveDateTime("dtpotu", dtpotu);
                SaveDateTime("dtfaya", dtfaya);
                SaveDateTime("dtzhangye", dtzhangye);
                SaveDateTime("dtkaihua", dtkaihua);
                SaveDateTime("dtjieguo", dtjieguo);
