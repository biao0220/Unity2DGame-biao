...
public class Tudi1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dt3 = LoadDateTime("tu11");
        dtpotu = LoadDateTime("dtpotu1");
        dtfaya = LoadDateTime("dtfaya1");
        dtzhangye = LoadDateTime("dtzhangye1");
        dtkaihua = LoadDateTime("dtkaihua1");
        dtjieguo = LoadDateTime("dtjieguo1");

        type = PlayerPrefs.GetInt("type1");

        dt1 = DateTime.Now;


      Pet1 = LoadGameObject("Pet11");
        if (Pet1 != null)
        {
            Pet1mount = false;
    ...
    void Update()
    {
        ...
        if (type == 0)
        {
            ...
            if (Input.GetKeyDown(KeyCode.F) && inTudi)
            {
                ...
                dt3 = DateTime.Now.AddSeconds(50f);
                SaveDateTime("tu11", dt3);

                dtpotu = dt3.AddSeconds(-40f);

                dtfaya = dt3.AddSeconds(-30f);
                dtzhangye = dt3.AddSeconds(-20f);
                dtkaihua = dt3.AddSeconds(-10f);
                dtjieguo = dt3;
                SaveDateTime("dtpotu1", dtpotu);
                SaveDateTime("dtfaya1", dtfaya);
                SaveDateTime("dtzhangye1", dtzhangye);
                SaveDateTime("dtkaihua1", dtkaihua);
                SaveDateTime("dtjieguo1", dtjieguo);
